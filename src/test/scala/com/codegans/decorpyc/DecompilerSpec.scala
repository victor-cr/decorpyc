...
//    private val versionDirs: List[File] = List(new File(rootDir, "v8.4").getAbsoluteFile.getCanonicalFile) // rootDir.listFiles(versionFilter).map(_.getAbsoluteFile.getCanonicalFile).toList
